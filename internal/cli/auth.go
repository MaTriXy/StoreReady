...
The session is saved locally at ~/.storeready/config.json.`,
	RunE: runAuthLogin,
}
...
func runAuthLogin(cmd *cobra.Command, args []string) error {
	...
	fmt.Println()
	fmt.Println("  Run 'storeready scan --app-id YOUR_APP_ID' to start scanning.")
	fmt.Println()
...
func runAuthStatus(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		fmt.Println("\n  Not authenticated.")
		fmt.Println("  Run 'storeready auth login' (Apple ID) or 'storeready auth setup' (API key).")
		return nil
	}

	purple.Println("\n  storeready auth status")
	...
	case config.AuthMethodSession:
		...
		if cfg.Session != nil {
			...
			if cfg.Session.ExpiresAt.After(time.Now()) {
				...
				fmt.Println("  Status:  EXPIRED — run 'storeready auth login' to re-authenticate")
			}
		}
	...
	default:
		fmt.Println("  Unknown auth method. Run 'storeready auth login' to set up.")
	}
