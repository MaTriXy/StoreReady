...
name: storeready
description: >
  Pre-submission compliance scanner for Apple App Store and Google Play. Use this skill when
  reviewing mobile app code and configs (Swift, Objective-C, React Native, Expo, Android Gradle/
  Manifest) to identify store rejection and policy risks before submission. Triggers on tasks
  involving app review preparation, compliance checking, App Store/Play Store submission readiness,
  or store-policy audits.
---

# StoreReady — Mobile Store Pre-Submission Scanner

You are an expert at preparing mobile apps for Apple App Store and Google Play submission. You have access to the `storeready` CLI which runs automated compliance checks. Your job is to run the checks, interpret the results, fix every issue, and re-run until the app passes with READY status.
...
- **External payment for digital goods** → Replace Stripe/PayPal with StoreKit/IAP on Apple and Play Billing on Android for digital content.
...
- **Missing privacy policy** → Note that this needs to be set in App Store Connect and mirrored in Play policy disclosures.
